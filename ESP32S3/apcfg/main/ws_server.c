...
esp_err_t handle_ws_req(httpd_req_t *r)
{
    ...
    if (buf == NULL) {
        ESP_LOGE(TAG, "Failed to allocate memory for ws receive");
    ...
    if(ret == ESP_OK)
    {
        ...
        {
            ESP_LOGI(TAG, "Got packet with message: %s", pkt.payload);
            pkt.payload[pkt.len] = 0; //null-terminate whatever we received and treat like a string...
            if(ws_receive_fn) ws_receive_fn(pkt.payload,pkt.len);
        }
